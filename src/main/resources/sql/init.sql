...
CREATE TABLE `id_gen_2024` (
                             `id` VARCHAR(100) PRIMARY KEY,
                             `year` VARCHAR(4) NOT NULL,
                             `create_time` datetime DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='id生成表';

CREATE TABLE `id_gen_2025` LIKE `id_gen_2024`;
CREATE TABLE `id_gen_2026` LIKE `id_gen_2024`;
